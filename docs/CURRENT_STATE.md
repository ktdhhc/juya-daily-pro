...
3. **手机端适配（<640px 断点）**：报头拆两行——顶行品牌 + 图标（搜索/同步/合订本/复制/主题），第二行 `.m-nav` 横滚导航（日报/事件流/公司/面板 + 审核/管理），桌面端 `.nav-link` 三联不变；**报头全站冻结**（sticky，见下方分层注意）；`/stream` 左栏在手机端改底部抽屉（`筛选` 按钮 → `#stream-filter-sheet` z-[60]，选公司/分类即收起）；公司索引手机端两列；`html/body` 横向裁剪用 `overflow-x: clip`（**不可改回 hidden**：hidden 会让 html/body 成滚动容器，iOS 下窗口滚动失效、root=null 的 IntersectionObserver 永不触发 → 公司页/事件流滚动加载不动、sticky 失效）；滚动加载另有 capture 阶段 scroll 兜底（哨兵视口坐标判定）。Tailwind `hidden`/`sm:hidden` 会被非分层的自定义类（`.icon-btn`/`.m-nav`/`.facet-chip`）盖过，响应式显隐用 `.only-mobile`/`.only-desktop`。**同类坑：`.site-header` 不写 `position`（曾写 `position: relative` 盖掉 Tailwind 的 `sticky top-0 z-50`，导致除日报页外所有页面报头随滚动移走）；`globals.css` 也不要手写 `-webkit-backdrop-filter` 前缀——手写前缀会让 Lightning CSS 丢掉标准 `backdrop-filter`（编译产物只剩 -webkit-，Chromium 下模糊失效）。**
...
- 设计契约：`docs/FRONTEND_DESIGN.md`（§4.8 数据面板、§4.10 时间线、§4.11 搜索、§4.12 审核台）；原型 `demo/index.html`。
