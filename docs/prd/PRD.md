...
把 [daily.juya.uk](https://daily.juya.uk) 每日发布的 AI 资讯合集，按"事件流"与"公司"两个维度重新整理，部署在 Cloudflare 上供多用户查阅，并为未来引入 AI 问答 / RAG 预留语义索引入口。
...
把 Daily Issue 拆解成可独立查询的 Item 原子单位，存入 Cloudflare D1，并按"事件流"和"公司"两个新视图对外提供查询：

- **Item** 是数据原子单位，每个 `#N` 对应一条，含标题、主链接、摘要、正文、相关链接、归属 Category 与 Company。
- **事件流视图 `/stream`** 按天分组的 Item 卡片流，左侧 facet 可按公司、分类、日期范围筛选，支持全文检索。
...
30. 作为关注一致性的用户，我想首页与事件流/公司页走同一条 Worker read API，避免"今天的事件在公司页和事件流缺、首页能看到"的口径分裂。
...
- 事件流首屏默认时间窗口：近 7 天；用户向下滚动到底自动加载更早一期，无限延展至历史最远；facet 筛选状态改变时清空已加载列表、按新筛选条件重新从近 7 天加载
- 事件流卡片字段：日期 / 公司徽章（多家并排主公司略大）/ 分类小标 / 标题 / 主链接 ↗ / 摘要 / 相关链接数量 / 公司数 +N 提示 / Role 提示 / 跳转公司页 / 跳转日期阅读页"看原期"
...
- 公司页下方：该公司 Item 按时间倒序展示。复用 `/stream` 同款卡片组件，但按"已在公司页语境下"做差异渲染：
  ...
  - 其余字段（日期 / 分类 / 标题 / 主链接 / 摘要 / 相关链接数 / Role 提示 / "看原期"）正常渲染
